...
const MovieSlider = ({category}) => {
    ...
    const sliderRef = useRef(null)   //parent referrence for slider

    const fromattedCategoryName = category.replaceAll("_", " ")[0].toUpperCase() + category.replaceAll("_", " ").slice(1);
  ...
  return (
    <div className='bg-black text-white relative px-5 md:px-20' onMouseEnter={() => setShowArrows(true)} onMouseLeave={() => setShowArrows(false)}>
        <h2 className='mb-4 text-2xl font-bold'>
            {fromattedCategoryName} {formattedContentType}
        </h2>
